...
def load_lidar_file(file_path):
    ...
    #     return points
    if ext == '.bin':
        if 'nuscenes' in file_path.lower():
...
def get_bbox_top_center(bbox_tensor):
    ...
    # print ("----------top-----------",len(bbox_tensor))
    if len(bbox_tensor) == 9:
        x,y,z,dz,dy,dz,heading,vx,vy =bbox_tensor
    elif len(bbox_tensor) == 7:
        x,y,z,dx,dy,dz,heading = bbox_tensor
    else:
        raise ValueError(f"Unexpected bbox format: {len(bbox_tensor)}values" )
